...
namespace MinStat.Enterprises.DAL
{
    ...
    public class EnterpriseDataRepository : IEnterpriseDataRepository
    {
        ...
        public void UpdatePerson(int personId, int activityId, string title, string post, int postLevelId, int educationLevelId, decimal yearSalary, bool gender, bool wasQualificationIncrease,
          bool wasValidate, int birthYear, int hiringYear, int startPostYear, int dismissalYear)
        {
            #region Pre-conditions
        ...
        public IEnumerable<PostLevel> GetPostLevels()
        {
        ...
        public void CreatePersons(IEnumerable<Person> persons)
        {
            var activities = persons.Select(x => x.ActivityId).Distinct().ToList();
            int summaryId = persons.First().SummaryId;
            foreach (int activity in activities)
            {
                if (!_context.SummaryActivities.Any(x => x.ActivityId == activity && x.SummaryId == summaryId))
                {
                    SummaryActivity sa = new SummaryActivity();
                    sa.ActivityId = activity;
                    sa.SummaryId = summaryId;
